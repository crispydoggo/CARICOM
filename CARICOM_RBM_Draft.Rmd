...
RBM <- read_excel("RBM_CXC.xlsx")
RBM_ideal <- read_excel("RI_ideal.xlsx")

RBM_ideal <- RBM_ideal %>%  
            mutate( dimension = case_when(  
              dimension_d ==  "Institutionalization" ~ 1,
              dimension_d ==  "Implementation"   ~ 2,
              dimension_d ==  "Technical" ~ 3,
              dimension_d ==  "Use" ~ 4
              ) ) %>% 
...
# Intento de viñetitas


### Intento {.tabset .tabset-pills}

## Muchas palabras {-}

## Muchas muchas palabras {-}

## Muchas muchas muchas palabras {-}

## Muchas muchas muchas muchas muchas muchas palabras {-}
...
for ( i in 1:4){
  ...
  for( j in min(stage_1$bullet_id):max(stage_1$bullet_id)){
    ...
    for( k in 1:max(stage_2$sub_id)){
      ...
      # Cambiar el código con el label del sub elemento 
      
      # El número de sub-elemento (1.1) / label
      # CARICOM -> Final Reports -> Sub elements short names ( R1 as well)
